...
| WarnOnReboot | SwitchParameter | false | False | Checks if there is a pending reboot on the system to finalize Windows Updates and returns<br /> warning if one is pending |
| CritOnReboot | SwitchParameter | false | False | Checks if there is a pending reboot on the system to finalize Windows Updates and returns<br /> critical if one is pending |
